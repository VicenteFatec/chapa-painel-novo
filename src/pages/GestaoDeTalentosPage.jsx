...
function GestaoDeTalentosPage() {
    ...
    const abrirModalEdicao = (trabalhador) => { setTrabalhadorEmEdicao(trabalhador); setFormData({ nomeCompleto: trabalhador.nomeCompleto || '', cpf: trabalhador.cpf || '', rg: trabalhador.rg || '', telefone: trabalhador.telefone || '', status: trabalhador.status || 'Disponível', fotoURL: trabalhador.fotoURL || '', dossieURL: trabalhador.dossieURL || '', dataNascimento: formatarDataParaInput(trabalhador.dataNascimento), }); setArquivoParaExcluir({ foto: null, dossie: null }); setUploadProgress(''); setIsModalOpen(true); };
    ...
    const handleFormSubmit = async (e) => { e.preventDefault(); if (trabalhadorEmEdicao) { await handleUpdateChapa(); } else { await handleCreateChapa(); } };
    const handleCreateChapa = async () => { if (!formData.nomeCompleto) { return; } setUploadProgress('Salvando...'); try { await excluirArquivoDoStorage(arquivoParaExcluir.foto); await excluirArquivoDoStorage(arquivoParaExcluir.dossie); const dataNascimentoTS = parseDateFromInput(formData.dataNascimento); const dadosParaSalvar = { ...formData, dataImportacao: Timestamp.now(), regiao: getRegionFromDDD(formData.telefone), dataNascimento: dataNascimentoTS }; await addDoc(chapasCollectionRef, dadosParaSalvar); fecharModal(); fetchChapas(); } catch (error) { console.error("Erro ao criar: ", error); } finally { setUploadProgress(''); } };
    const handleUpdateChapa = async () => { if (!trabalhadorEmEdicao?.id) return; setUploadProgress('Atualizando...'); try { await excluirArquivoDoStorage(arquivoParaExcluir.foto); await excluirArquivoDoStorage(arquivoParaExcluir.dossie); const dataNascimentoTS = parseDateFromInput(formData.dataNascimento); const dadosParaAtualizar = { ...formData, regiao: getRegionFromDDD(formData.telefone), dataNascimento: dataNascimentoTS, }; const docRef = doc(db, "chapas_b2b", trabalhadorEmEdicao.id); await updateDoc(docRef, dadosParaAtualizar); fecharModal(); fetchChapas(); } catch (error) { console.error("Erro ao atualizar: ", error); } finally { setUploadProgress(''); } };
    ...
    if (isLoading) {
        ...
        tableContent = (<tr><td colSpan="4" style={{ textAlign: 'center', padding: '2rem' }}>Nenhum trabalhador encontrado.</td></tr>);
    }
